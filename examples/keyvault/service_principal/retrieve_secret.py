...
def retrieve_storage_account_info(config):
    ...
    sa_name_bundle = client.get_secret(vault.properties.vault_uri, config['STORAGE_ACCOUNT_KV_BASE'] + '_NAME')
    sa_key_bundle = client.get_secret(vault.properties.vault_uri, config['STORAGE_ACCOUNT_KV_BASE'] + '_KEY')
